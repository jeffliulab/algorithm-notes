...
1. **主食餐台 (`nums1`)** : 上面摆着 `米饭(1元)`, `牛排(7元)`, `龙虾(11元)`。菜是按价格**从低到高**摆好的。
2. **配菜餐台 (`nums2`)** : 上面摆着 `青菜(2元)`, `土豆泥(4元)`, `例汤(6元)`。也按价格**从低到高**摆好了。
...
1. 保持 **米饭** 不变，配菜换成下一个（土豆泥）。
2. 保持 **青菜** 不变，主食换成下一个（牛排）。
...
1. 保持 **米饭** 不变，配菜再换下一个（例汤）。
2. 保持 **土豆泥** 不变，主食换成下一个（牛排）。
...
1. 从最便宜的那个开始。
2. 选定一个组合后，只考虑跟它**相邻**的两个新组合。
3. 把这些新组合扔进一个“候选池”（小明的记事本）。
4. 然后永远从“候选池”里挑出最便宜的那个。

这个过程保证了我们每次拿到的都是**当前所有可能性中**最便宜的，所以我们拿 K 次，就一定是前 K 个最便宜的。
